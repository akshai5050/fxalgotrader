...
add_to_predict_window:{
	...
	$[(count lis[0;`pre]) < 20; 
		lis[0;`pre],::val;
		[lis[0;`dt]:x[`dt];
			`nnet_predictions insert (x[`dt]; val; predict[lis[0;`pre]]);
			`svr_predictions insert (x[`dt]; val; predictSvr[lis[0;`pre]]);
			`arima_predictions insert (x[`dt]; val; predictArima[lis[0;`pre]]);
			if[(count nnet_predictions) > 20;
				0N!x[`dt], count nnet_predictions;
				`final_predictions insert (x[`dt]; val; combined_predict[-20#arima_predictions[`predictions];-20#nnet_predictions[`predictions];-20#svr_predictions[`predictions];-20#arima_predictions[`actual]])];
				lis[1;`pre]::1_lis[0;`pre];
				lis::1_lis;
				lis,::(`dt`pre)!()();
				lis[0;`pre],::val;
				publish_nnet_web[];
				...
				publish_arima_web[];
				publish_final_web[];
				if[(count final_predictions)>=21;
					tradingStrategy[]];
				publish_nnet_errors_web[x[`dt]];
				publish_svr_errors_web[x[`dt]];
				publish_arima_errors_web[x[`dt]];
				publish_combined_errors_web[x[`dt]]]]}
